...
public class AsepriteData
{
	...
	private void GenerateFramesData()
	{
		...
		foreach (var frame in AsepriteFrames)
		{
			...
			var palette = ExtractPalette(frame);
			var layers = ExtractChunks<AsepriteLayerChunkType>(frame);
...
internal static class AsepriteExtensions
{
	...
	public static void CopyFrom(this AsepriteData.AsepriteImage image, byte[] buffer, int pixelSize)
	{
		var offset = 0;

		for (var y = 0; y < image.Height; y++)
		{
			var row = new AsepriteData.AsepriteImage.Row();
			image.Add(row);
