...
public class PaletteMerger4Bit : PaletteMerger
{
	...
	private void ValidateColoursPerImage(IReadOnlyList<ImageData> images)
	{
		...
		foreach (var image in images)
		{
			...
			{
				throw new Invalid4BitPaletteException($"Image {index} requires {count} colours, 4-bit mode only supports up to {MaxColoursPerBank}!");
			}
		}
	}
	...
	private List<ColourBank> MapImagesIntoBanks(IReadOnlyList<ImageData> images)
	{
		...
		{
			throw new Invalid4BitPaletteException($"Couldn't fit all chars into {MaxBanks} banks ({result.Count} banks are needed)");
		}
