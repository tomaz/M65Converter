...
public static class CommandExtensions
{
	...
	public static int InvokeAllCommands(this Command command, string[] args)
	{
		...
		Console.WriteLine($"Matching {subcommands.Length} possible commands with {args.Length} arguments");
		...
		while (index < args.Length)
		{
			...
			Console.WriteLine();
			Console.WriteLine($"Running {string.Join(' ', runArgs)}");
		...
		// If we reached here, all command completed successfully.
		return 0;
	}
}
