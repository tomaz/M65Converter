...
public class ScreenLookupExporter : BaseExporter
{
	...
	{
		var bytesPerCharWidth = Data.GlobalOptions.CharInfo.BytesPerCharIndex;
		...
		void AddLookupTable(ScreenExportData.Row row, int layer, int column)
		{
			var baseAddress = screenStartAddress + column * bytesPerCharWidth;
			var shiftBits = 0;
			
			var descriptions = new string[] { "Byte 0 (LSB)", "Byte 1", "Byte 2 (MSB)" };
			var dataColumn = column == 0 ? row.Columns[0] : row.Columns[column + 1];
			var titleType = dataColumn.IsSprite ? "Sprite" : "Screen";
			var titlePrefix = column switch
			{
				0 => "Start of screen row lookup table: ",
				_ => $"{titleType} {dataColumn.LayerName ?? $"{layer}"} screen row GOTOX lookup table: "
			};

			// Mega 65 can only have screen data anywhere in the first 384KB (0-$60000), so we need 3 bytes for these lookup tables.
		...
		for (var x = 0; x < sampleRow.Columns.Count; x++)
		{
			...
			switch (column.Type)
			{
				case ScreenExportData.Column.DataType.FirstData:
				case ScreenExportData.Column.DataType.Attribute:
					AddLookupTable(sampleRow, layerIndex, x);
					layerIndex++;
					break;
			}
		}
