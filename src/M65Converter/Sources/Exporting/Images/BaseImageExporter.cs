...
public abstract class BaseImageExporter
{
	...
	public class Measures
	{
		...
		public class BoxMeasurer
		{
			...
			/// <summary>
			/// Section title position, only available if title is used.
			/// </summary>
			public Point? SectionTitleLeftTop { get; private set; }
			...
			/// <summary>
			/// The height of the section title area and margin to data.
			/// 
			/// This takes into account chess pattern and inline-ness, so you can simply add this and <see cref="TitleHeight"/> to get the height of the whole title/section area.
			/// </summary>
			public int SectionTitleHeight { get; private set; }
			...
			/// <summary>
			/// Top header position, for first item only.
			/// </summary>
			public Point TopHeader
			{
				get => new(
					x: TopLeft.X,
			...
			/// <summary>
			/// Current item index (x, y) within the data (aka not the pixel coordinate, but 2D index within the box data). Useful for retrieving the data item for example.
			/// </summary>
			...
			private bool isTitleElevated = false;
			...
			/// </summary>
			public void UseTitle(bool isUsed = true, bool isElevated = false, int sectionRows = 1)
			{
				...
				if (isUsed == isUsingTitle && isElevated == isTitleElevated) return;

				// Calculate title height and margin between title and content.
				var titleSize = measures.FontRenderer.Measure("Xy");
				var titleMargin = measures.ElementMargin.Y;
				var textHeight = titleSize.Height;

				// Combined values for simpler code below.
				var titleAndMarginHeight = textHeight + titleMargin;
				var sectionAndMarginHeight = titleAndMarginHeight * sectionRows;

				// If title is not used, reset the value. We must also reduce top left accordingly.
				if (!isUsed)
				{
					TitleLeftTop = null;
					SectionTitleLeftTop = null;
					TitleHeight = 0;
					SectionTitleHeight = 0;
					topLeft.Y -= titleAndMarginHeight + (isTitleElevated ? sectionAndMarginHeight : 0);
					isUsingTitle = false;
					isTitleElevated = false;
					return;
				}

				// Setup title and section area heights.
				TitleHeight = textHeight;
				SectionTitleHeight = sectionAndMarginHeight;
				...
				// Adjust top left to account for the title.
				topLeft.Y += titleAndMarginHeight;

				// Prepare position for section titles.
				SectionTitleLeftTop = new Point(
					x: topLeft.X,
					y: isElevated ? topLeft.Y : TitleLeftTop.Value.Y
				);

				// If title is elevated, move top-left down again to account for additional line with section titles.
				if (isElevated)
				{
					// For elevated section titles, we should simply add the whole section area height.
					topLeft.Y += sectionAndMarginHeight;
				}
				else
				{
					// For inline section titles, we should add "rows - 1" height (so basically section height - title height which is shared with top sections). We should also remove title height from section height variable to properly account for the vertical overlap.
					topLeft.Y += sectionAndMarginHeight - titleAndMarginHeight;
					SectionTitleHeight -= titleAndMarginHeight;
				}

				isUsingTitle = true;
				isTitleElevated = isElevated;
			}
